...
void removeElement(int position) {
    ...
    if (position == 0) {
        temp = top;
        top = temp->next;
        if (top != nullptr) { // If there is a new top, set its prev to nullptr
            top->prev = nullptr;
        } else {
            tail = nullptr; // If the list is empty now
        }
        free(temp);
        return;
    }
        ...
        if (tail != nullptr) {
            ...
            top = nullptr; // If the list is empty now
        }
        free(temp);
        return;
    }
    ...
    if((size/2) <= position) {
        temp = tail; // Start from tail for larger index
...
int main() {
    ...
    //insert element at the end of the list and display accordingly
    insertLast(3);
    cout << "\nList after inserting 3 at the end:" << endl;
    printList();


    //print the size of the list after modifying it
    ...
    */
    ...
    //use get to find 2nd and 4th element
    cout << "\nSecond element is: " << get(1) << endl;
    cout << "First element is: " << get(0) << endl;

    //remove first and second element and display accordingly
    removeElement(0);
    removeElement(1);
    cout << "\nList after removing first and second element:" << endl;
    printList();
